...
class CEvent
{
    ...
    public $server_url = 'https://cremaprodotti.com.ua/Bitrix24/skk/liqpay2/callback.php';
    ...
    public function manageEvent($event, $params){
        ...
        case 'lead':// запрос полей лида
            ...
            writeToLog($result_lead,"result_lead");
            ...
            //запись в базу стадии лида
            try {
                $res_db = $db->query('INSERT INTO `deal_field` (PORTAL, ITEM, ITEM_ID, OLD_ROW_FIELD)
                                VALUES (?s, ?s, ?i,?s)' .
                    ' ON DUPLICATE KEY UPDATE  `OLD_ROW_FIELD`=?s',
                    $params['auth']['domain'], $event, $params['data']['FIELDS']['ID'], $result_lead['result']['STATUS_ID'],
                    $result_lead['result']['STATUS_ID']);
            ...
            if ($result_lead['result']['STATUS_ID'] == $old_stage_id){
            ...
            $lead_stage_id = $result_lead['result']['STATUS_ID'];
            $contact_id =    $result_lead['result']['CONTACT_ID'];
            $currency_id = $result_lead['result']['CURRENCY_ID'];
           ...
           /* elseif ($amount_db == $result_lead['result']['OPPORTUNITY']){
                ...
                if ($isPayed = $this->checkPaidPeriod($obB24, $result_db)) {
                    ...
                    $description = $order_text.' № ' . $result_lead['result']['ID'];
                    // Если поле аванс заполнено - больше 0 - в лик пей передаем єту сумму
                    if ($result_lead['result'][$avans_lead_id] >0){
                        $ammount =  $result_lead['result'][$avans_lead_id];
                        $description = $description.' Аванс';

                    }
                    else{
                        $ammount =  $result_lead['result']['OPPORTUNITY'];
                    }


                    $order_id = $params['auth']['domain'] . "_LEAD_" . $result_lead['result']['ID'] . "_" . $ammount;
                    ...
                    if (($method_link) OR ((!$method_link)AND(!$method_bot)AND(!$method_email))) {
                        $this->lead_link = $link;
                       if( $result_lead_update = $obB24->call('crm.lead.update', array(
                            'id' => $result_lead['result']['ID'],
                            'fields' => array(
                                $lead_liq_pay => $link)
                        ))) {
                    ...
                    if ((!$result_item['ACTUAL']) OR ($result_item['ACTUAL'] == 1)) { //первый раз сущность обрабатывается или не опалчена в success
                        ...
                        try {
                            $res_db = $db->query('INSERT INTO `deal_field` (PORTAL, ITEM, ITEM_ID, STATUS, PAYMENT_ID,	OLD_ROW_FIELD, OLD_AMOUNT, ACTUAL)
                                ...
                                $params['auth']['domain'], $event, $params['data']['FIELDS']['ID'], 'initial', 0, $result_lead['result']['STATUS_ID'], $result_lead['result']['OPPORTUNITY'], 1,
                                $event, $params['data']['FIELDS']['ID'], 'initial', 0, $result_lead['result']['STATUS_ID'], $result_lead['result']['OPPORTUNITY'], $actual);
                        } catch (Exception $error) {
                            $this->returnJSONResult(array('status' => 'error', 'result' => $error->getMessage()));
                        }
                    }
        ...
        case 'deal':
            ...
            try {
                $res_db = $db->query('INSERT INTO `deal_field` (PORTAL, ITEM, ITEM_ID, OLD_ROW_FIELD)
                                VALUES (?s, ?s, ?i,?s)' .
                    ' ON DUPLICATE KEY UPDATE  `OLD_ROW_FIELD`=?s',
                    $params['auth']['domain'], $event, $params['data']['FIELDS']['ID'], $result_deal['result']['STAGE_ID'],
                    $result_deal['result']['STAGE_ID']);
            } catch (Exception $error) {
                $this->returnJSONResult(array('status' => 'error', 'result' => $error->getMessage()));
            }
            ...
            if ($result_deal['result']['STAGE_ID'] == $old_stage_id){
               /* $im_user = $obB24->call( "im.notify", array(
                    "to" => $this->user,
                ...
                writeToLog("result_deal['result']['STAGE_ID'] == old_stage_id");
           ...
           /* if ($deal_stage_id == $old_stage_id) {
                writeToLog('стадия не менялась - выход ');
                die;
            }*/
            ...
            /*}elseif ($amount_db == $result_deal['result']['STAGE_ID']){
            ...
            */
            /*elseif ($amount_db == $result_deal['result']['OPPORTUNITY']){
                ...
                if ($isPayed = $this->checkPaidPeriod($obB24, $result_db)) {
                    ...
                    $order_id = $params['auth']['domain'] . "_DEAL_" . $result_deal['result']['ID'] . "_" . $ammount;
                    $expired_date  = date("Y-m-d H:i:s",strtotime("+ 3 day"));
